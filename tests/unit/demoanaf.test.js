...
describe('scraper/anaf.js', () => {
  ...
  describe('searchCompany', () => {
    it('should return array of companies for valid brand', async () => {
      mockFetch.mockResolvedValue(anafSearchResponse([
        { cui: 51981214, name: 'TAKTILE S.R.L.', statusLabel: 'Funcțiune' }
      ]));

      const results = await anaf.searchCompany('TAKTILE');
    ...
    it('should fallback to CUIFirma when ANAF search fails', async () => {
      mockFetch
        ...
        .mockResolvedValueOnce({ ok: true, json: async () => ({ results: [{ cui: 51981214, name: 'TAKTILE S.R.L.', is_active: true }] }) });

      const results = await anaf.searchCompany('TAKTILE');

      expect(Array.isArray(results)).toBe(true);
      expect(results.length).toBeGreaterThan(0);
      expect(results[0].cui).toBe('51981214');
    });
    ...
    it('should encode brand name in URL', async () => {
      ...
      await anaf.searchCompany('TAKTILE SRL');
      expect(capturedUrl).toContain(encodeURIComponent('TAKTILE SRL'));
    });
  });

  describe('getCompanyFromANAF', () => {
    it('should return company data for valid CIF', async () => {
      mockFetch.mockResolvedValue(anafCompanyResponse(TAKTILE_ANAF_RECORD));

      const data = await anaf.getCompanyFromANAF('51981214');

      expect(data).toBeDefined();
      expect(data.cui).toBe(51981214);
      expect(data.name).toBe('TAKTILE S.R.L.');
    ...
    it('should throw when both ANAF and CUIScan fail', async () => {
      ...
      await expect(anaf.getCompanyFromANAF('51981214')).rejects.toThrow();
      expect(mockFetch).toHaveBeenCalledTimes(2);
    });
  ...
  describe('getCompanyFromANAFWithFallback', () => {
    it('should return fresh data when API works', async () => {
      ...
      const data = await anaf.getCompanyFromANAFWithFallback('51981214');
    ...
    it('should use cached data when API fails', async () => {
      ...
      const data = await anaf.getCompanyFromANAFWithFallback('51981214', CACHED_DATA);
    ...
    it('should throw when API fails and no cache available', async () => {
      ...
      await expect(anaf.getCompanyFromANAFWithFallback('51981214')).rejects.toThrow();
    });
  });
});
