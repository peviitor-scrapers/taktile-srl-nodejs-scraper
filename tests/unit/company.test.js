...
const TAKTILE_ANAF_RECORD = {
  cui: 51981214,
  name: 'TAKTILE S.R.L.',
  address: 'MUNICIPIUL IAŞI, IAŞI',
  caenCode: '6210',
  inactive: false,
  vatRegistered: true,
  eFacturaRegistered: false,
  headquartersAddress: { locality: 'MUNICIPIUL IAŞI' }
};
...
describe('company.js', () => {
  ...
  describe('getCompanyData (no cache)', () => {
    it('should fetch TAKTILE via direct CIF lookup and return company data', async () => {
      mockFetch.mockResolvedValueOnce(anafCompanyResponse(TAKTILE_ANAF_RECORD));

      const result = await company.getCompanyData();

      expect(result).toHaveProperty('company', 'TAKTILE S.R.L.');
      expect(result).toHaveProperty('cif', '51981214');
  ...
  describe('getCompanyData (with cache)', () => {
    ...
    it('should use cached company data when available', async () => {
      ...
      expect(result.cif).toBe('51981214');
  ...
  describe('validateAndGetCompany', () => {
    ...
    it('should return company data with status active', async () => {
      mockFetch
        .mockResolvedValueOnce(anafCompanyResponse(TAKTILE_ANAF_RECORD))
        .mockResolvedValueOnce(solrResponse(5, [
          { url: 'https://test.com/1', title: 'Job 1' },
          { url: 'https://test.com/2', title: 'Job 2' }
        ]))
        .mockResolvedValueOnce(peviitorResponse([{ company: 'TAKTILE S.R.L.' }]));
    ...
    // TAKTILE e activă — testul inactive se rulează doar dacă firma e inactivă
    if (TAKTILE_ANAF_RECORD.inactive) {
      it('should return inactive status when company is inactive', async () => {
        const inactiveRecord = { ...TAKTILE_ANAF_RECORD, inactive: true };
